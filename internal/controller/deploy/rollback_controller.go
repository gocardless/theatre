...
func (r *RollbackReconciler) pollDeploymentStatus(ctx context.Context, logger logr.Logger, rollback *deployv1alpha1.Rollback) (ctrl.Result, error) {
	// For now, we use the DeploymentURL as a proxy - in a real implementation you'd store the deployment ID
	// This is a simplified approach; you may want to add a DeploymentID field to RollbackStatus
	statusResp, err := r.Deployer.GetDeploymentStatus(ctx, rollback.Status.DeploymentURL)
	if err != nil {
