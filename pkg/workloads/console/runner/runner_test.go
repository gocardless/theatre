...
var _ = Describe("Runner", func() {
	...
	Describe("CreateResource", func() {
		Context("When creating a new console", func() {
			...
			It("Creates the console via the clientset", func() {
				...
				err := theatreClient.List(
					context.TODO(),
					&csls,
				)
				Expect(err).NotTo(HaveOccurred(), "failed to list consoles")
				Expect(csls.Items).To(HaveLen(1), "only one console should be present")
			})
			...
			It("Creates the console in the namespace specified", func() {
				...
				err := theatreClient.Get(
					context.TODO(),
					client.ObjectKey{
						Name:      "",
						Namespace: createdCsl.Namespace,
					},
					&csl,
				)
				Expect(err).NotTo(HaveOccurred(), "failed to get console")
				Expect(csl.Namespace).To(Equal(namespace), "namespace should match the creation namespace")
			})
			...
			It("Returns an error when the console already exists", func() {
				...
				Expect(err).To(MatchError(
					errors.NewAlreadyExists(schema.GroupResource{Group: "workloadsv1alpha1", Resource: "consoles"}, ""),
				))
			})
		})
	})
	...
	Describe("WaitUntilReady", func() {
		...
		updateConsolePhase := func(in workloadsv1alpha1.Console, phase workloadsv1alpha1.ConsolePhase) {
			...
			err = theatreClient.Update(context.TODO(), &csl)
			...
			Context("When the console is pending", func() {
				...
				Context("When phase is updated to Running", func() {
					It("Returns successfully", func() {
						...
						defer cancel()
						upToDateCsl, err := runner.WaitUntilReady(ctx, csl, true)
			...
			Context("When console does not exist", func() {
				...
				Context("But it is later created", func() {
					createCsl := func() {
						...
						newCsl := csl.DeepCopy()
						newCsl.Status.Phase = workloadsv1alpha1.ConsoleRunning
						err := theatreClient.Create(context.TODO(), newCsl)
