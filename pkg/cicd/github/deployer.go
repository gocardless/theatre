...
func (d *Deployer) TriggerDeployment(ctx context.Context, req cicd.DeploymentRequest) (*cicd.DeploymentResult, error) {
	...
	description := fmt.Sprintf("Rollback to %s: %s", req.Rollback.Spec.ToReleaseName, req.Rollback.Spec.Reason)
...
func (d *Deployer) buildPayload(req cicd.DeploymentRequest) map[string]interface{} {
	payload := map[string]interface{}{
		...
		"is_rollback":   true,
		"rollback_from": req.Rollback.Status.FromReleaseName,
		"rollback_to":   req.Rollback.Spec.ToReleaseName,
